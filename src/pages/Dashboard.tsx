...
export default function Dashboard() {
  ...
  if (overdue.length > 0)
    alerts.push({ tone: 'bad', text: `${overdue.length} pago(s) vencido(s) sin resolver.` })
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">
            {settings?.businessName} · {monthLabel(month)}
          </p>
        </div>
        ...
        <Card>
          ...
          {toCollect === 0 && toPay === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {overdue.length > 0 && (
                <p className="text-xs text-red-500">⚠️ {overdue.length} vencido(s). Míralos en Ingresos y gastos.</p>
              )}
            </div>
          )}
        </Card>
